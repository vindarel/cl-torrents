...
* Appendice A: the same, with CL21
  ...
  package of our code base if don't  want to use it everywhere, and we
...
- https://lispcookbook.github.io/cl-cookbook/cl21.html#mapping
  ...
  And, that's it, we didn't have  other conflicts and other test suite
  executed rightly.
...
** Benefits ?
   ...
   inconsistencies,  sometimes  the  firts   parameter  is  the  list,
   sometimes not.

   It just feels  wrong to me, specially today, and  specially when CL
   has this generic mecanism.

   So CL functions become generic:
...
- https://lispcookbook.github.io/cl-cookbook/cl21.html#generic-functions
  ...
  work with CL21, but we didn't have much room for improvement either.
  We made  the code compact  and clean already  (well, we hope  !  PRs
  welcome). So CL21's benefit would have been noticeable (and huge for
  us) during development.

  Allright, I'm  sold on CL21,  if it was  only for that,  but there's
  more. More functional verbs (very welcome),  shorter lambdas, sane hash-tables, and
  less important and optional stuff (built-in regexps,…).
