...
lquery  returns a  vector. So either we  transform  the result to a list  with
=coerce=, either we use =(map 'list  ...)=.  Also the name =mapcar= is
unusual.

We  could use  [[https://lispcookbook.github.io/cl-cookbook/cl21.html][cl21]]'s =map=,  which works  on vectors  too so  no more
questions, it will work.
...
- searching into a local Pirate Bay dump,
- some *cache*
- regular testing  against the website (straightforward  and to couple
  with the CI),
- sharing this program: a *standalone executable* ? using Roswell ?,
- a *command line tool*
- …

We'll  carry on  by building  a self-contained  executable, using  the
advantages of a  couple Lisp implementations and  parsing command line
arguments.
...
#+END_EXPORT
   ...
   The  condition   signaled  is   named  after   our  implementation:
...
#+END_SRC
    ...
    on other projects. We could something like (thanks [[https://github.com/fukamachi/clack/blob/master/src/clack.lisp#L36][Clack]]):
...
* Conclusion
  ...
  You can comment on the repository: https://github.com/vindarel/cl-torrents
