...
In part  1, "Make  it work",  we explore how  to do  *asynchronous web
scraping*.   We try  things out  at  the repl  and we  create a  first
package. We  learn basic  *data structures* and  their gotchas  in the
way, we  also see where  to find documentation, some  useful libraries
and beginner-friendly tips.

Part 2 is a short introduction to the CL Object System (*CLOS*), which
is much  more a  pleasure to  work with than  dealing with  alists and
other data structures.

In part 3, "*Tests  and CI*", we write our first  unit test, with mocks,
which gives  us the opportunity to  write a first macro,  and we setup
Continuous Integration on Gitlab CI.

Part 4 is about  *building and delivering self-contained executables*,
*parsing command line arguments* and  such. While it was necessary for
us at the time to gather this  information, it is now available in the
[[https://lispcookbook.github.io/cl-cookbook/scripting.html][Common Lisp  Cookbook]] and is probably  more up to date.  We advise the
reader to go there.
...
Part 6  is about  *error handling* or  rather condition  handling.

Part 7, "More features", shows only a cache system at the moment.

Finally, we have a look at [[http://cl21.org/][CL21]].
...
A little explanation for =lquery=: the last =(text)= part is
...
[[https://github.com/hipeta/arrow-macros][arrow-macros]] defines  a few  threading macros. The  classic one  would be
...
dependencies (like a virtual env), see the [[https://lispcookbook.github.io/cl-cookbook/getting-started.html#advanced-dependencies-management][Cookbook]].
...
__Note__: We  can also truncate  the titles  and add an  ellipsis with
=str:prune=:
...
- getting more content (seeders, leechers): this is done in the app.
...
* Designing the scrapers with CLOS
...
=assoc-value= from Alexandria or loosing our tests when we changed the
...
You'll find more learning resources in the [[https://lispcookbook.github.io/cl-cookbook/clos.html][Cookbook]].
...
#+BEGIN_SRC text
(defclass <name> (list of super-classes)
  (list of slots of the form
    (name attributes))
 (optionally class options))
#+END_SRC

An object is created with =make-instance 'class=, but is good practice
to define a constructor like =make-torrent=.

The   =:initarg=  is   the  keyword   argument  to   the  constructor:
=(make-instance 'torrent :title "foo")=, the accessor is both a getter
and a setter:

#+BEGIN_SRC lisp
(defvar *foo* (make-instance 'torrent :title "foo" :href "http://" :source :test))
(title *foo*) ;; => "foo"
(setf (title *foo*) "bar")
#+END_SRC

=print-object= is for pretty printing. The line

: (defmethod print-object ((it torrent) stream)

/specializes/   the  =print-object=   generic  method   for  =torrent=
classes. It takes a required =stream= parameter.

See the actual code [[https://github.com/vindarel/cl-torrents/blob/master/src/models.lisp][here]] and how  it is used in scrapers [[https://github.com/vindarel/cl-torrents/blob/master/src/1337.lisp][here]].
...
#+END_SRC
   ...
   Here we give  a list to =torrents=  but I found useful
...
one. We should have a look to the new [[https://github.com/koji-kojiro/cl-repl][cl-repl]] too, an ipython-like repl.).
...
** with ASDF or Roswell
   ...
   with every  implementation, allows that  with the [[https://common-lisp.net/project/asdf/asdf.html#Convenience-Functions][make]]  command. It
...
#+END_SRC
   ...
   The =--non-interactive= option asks to  /not/ enter the debugger if
   an  error  were  to  happen,  and  exits  after  the  last  command
   (otherwise we would add a =(quit)=).

   Finally [[https://roswell.github.io][Roswell]], that  does a lot, also  has the =ros build=  command, that
   should  work  for  more   implementations.  It  is  a bit under-documented
   ([[https://github.com/roswell/roswell/issues/253][issue]]).
...
#+END_QUOTE
   ...
   [[https://hub.docker.com/r/daewok/lisp-devel/][lisp-devel  Docker image]]  (Debian's 13.x  is too  old, it  has ASDF
...
- [[https://github.com/mrkkrp/cl-ansi-term][cl-ansi-term]]  - print  colorized  text,  horizontal lines,  progress
  bars, list,  tables,…
- [[https://github.com/mrkkrp/cl-readline][cl-readline]] -  bindings to  the GNU Readline  library (Emacs  and Vim
  input   modes,  history,   basic   expansion, custom completion,…).  Example   project:
  [[https://github.com/koji-kojiro/cl-repl][cl-repl]]. Demo example: [[https://github.com/vindarel/cl-readline-example][cl-readine-example]].
  [[https://github.com/mrkkrp/shtookovina][shtookovina]] by the  same author is an example  command-line app that
  makes good use of both projects.
...
#+END_SRC
    ...
    [[https://z0ltan.wordpress.com/2016/08/06/conditions-and-restarts-in-common-lisp/][z0ltan.wordpress.com]]   will  tell   you  more,   and  looking   at
    =unix-opts= code may help (just keep if it's too much for now).

    This is  how it defines  a condition:  it first defines  a generic
    =troublesome-option= class, that  inherits from =simple-error= and
    has three slots, amongst which an =option= that will store the bad
    one.  Then  it  defines   =unknown-option=,  which  inherits  from
    =troublesome-option=.
...
#+END_SRC
   ...
   See [[https://github.com/mrkkrp/unix-opts/blob/master/unix-opts.lisp#L83][the source]].
...
#+END_SRC
    ...
    We chose to  just print "no results" and return  =nil= (the return
    value of =format=).  We could also print the error  message of the
    condition  (optional  argument  of  "error":  =(error  (condition)
    ...)=). We  might want to  not catch errors  when we are  in Slime
...
#+END_SRC
   ...
   the FS  storage is provided by  [[https://github.com/skypher/cl-store][cl-store]]. We could also  create an
   in-memory cache, suitable for memoization.

   The function =ensure-directories-exist=  creates a nested directory
   structure for us if needed. We  wrap its call in the =ensure-cache=
   function  in order  to  call  it at  the  beginning  of the  =main=
...
#+END_SRC
   ...
   Better yet, with an optional  =store= argument that defaults to the
   =*store*= parameter:
...
* Appendice A: the same, with CL21
  ...
  can still use regular CL symbols prefixed by =cl:=.
...
** Replacing CL by CL21

   We have to  use =cl21= instead of =cl=, and  =cl21-user= instead of
   =cl-user=.
...
#+END_SRC
   ...
   Our  =.asd= gained  a =cl21=  dependency.   This is  not enough  to
...
#+END_SRC
   ...
   here we compiled the =cl-torrents.lisp= sources (=C-c C-k)= and had
   an  error  on  the   debugger,  complaining  that  =mapcar=  didn't
   exist. Indeed, CL21 uses the classical =map= name instead. It still
   has  =mapcan=.  BTW,  "filter"  is  provided  by  =keep[-if][-not]=
   (instead of =remove[-if][not]=).

- https://lispcookbook.github.io/cl-cookbook/cl21.html#mapping
...
** Benefits ?
   ...
   I know  I have been frustrated  by =first= not being  generic, thus
   not working on arrays.  I wanted  to use it when trying things with
   =plump= and  =lquery=, but plump  returns an  array. And it  is the
...
- https://lispcookbook.github.io/cl-cookbook/cl21.html#generic-functions
  ...
  Same story  with =map= and =mapcar=.  You want to use  what is named
  =map= in  all other languages, but  you get error messages,  have to
  google  and learn  about =mapcar=  and =map  'list= with  a freaking
  first argument.
...
** Downsides ?
  ...
  though: the executable size. We had  one of *78Mo*, and the CL21 one
  is *93Mo*.
