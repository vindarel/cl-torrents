...
** with Buildapp and Roswell
   ...
   [[http://www.xach.com/lisp/buildapp/][Buildapp]]  is a  battle-tested  "application for  SBCL  or CCL  that
   configures and saves an executable Common Lisp image".
...
#+END_SRC
   ...
   Many applications use  it (for example, [[https://github.com/dimitri/pgloader][pgloader]]).  It is available
   on Debian: =apt install buildapp=.

   [[https://roswell.github.io][Roswell]], that  does a lot, also  has the =ros build=  command, that
   should  work  for  more   implementations.  It  is  not  documented
