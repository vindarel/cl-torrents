...
*** Configuration file (in development)

    cl-torrents will  read a =~./.torrents.conf= configuration
    file  from  three   locations:  cl-torrents  library's  directory,
    =~/.config/.torrents.conf= and =~/.torrents.conf=.  The last
...
#+END_SRC
  ...
  Your can set  them for all replic-like apps  in =~/.replic.conf=, or
